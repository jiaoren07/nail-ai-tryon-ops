...
┌─────────────────────────────────────────────────────┐
│  展现层 (React SPA)                                 │
│  ├── 共享入口：L0（/，双端分流 landing）            │
│  ├── 用户端视图：U0–U6（/gender, /upload, ...）    │
│  └── 运营端视图：O1–O7（/ops/*）                    │
└─────────────────────────────────────────────────────┘
                        │ HTTPS / JSON
┌─────────────────────────────────────────────────────┐
...
### 6.0 L0 双端入口（Shared Landing，独立于 U/O 编号）

- **路由**：`/`（用户首次进入站点的默认页）
- **职责**：让访客一眼看到产品双端，二选一进入对应流程；同时承担首屏品牌印象
- **页面结构**（按原型 Board 0）：
  - 顶部：品牌徽章「美甲 / AI 试戴让你更懂」+ 一句话定位
  - 主体：左右并排两张大卡片
    - **用户端 · AI 试戴美甲** → 点击跳转 `/gender`（U0 性别选择）
    - **运营端 · 智能运营助手** → 点击跳转 `/ops/overview`（O1 数据概览）
  - 卡片样式：白底大圆角 + 黄色 CTA 按钮 + 代表性预览图
- **session 处理**：进入本页时若 `sessionStorage.userId` 不存在则前端本地生成 UUID v4 并写入，后续 C 端请求均携带；运营端入口不强制 userId
- **降级**：直接访问 `/gender` / `/upload` / `/ops/*` 等深层路由仍然可达，L0 仅作首屏推荐入口

### 6.1 U0 性别选择页（用户端第一步）

- **路由**：`/gender`
- **前置条件**：无（任何时候进 /gender 都能用，无需先有 photoId）
...
- **路由**：`/upload`
- **前置条件**：必须已经在 U0 选过 `userGender`，否则重定向回 `/gender`
...
- **跳转**：成功上传/选定后，前端把 `photoId` 与 `handFeatures` 写入 Context，跳转 `/recommend`
- **session 兜底**：进入本页时若 `sessionStorage.userId` 仍不存在（极端情况：用户跳过 L0 直接访问 /upload），本地生成 UUID v4 写入
...
> O4 是报告生成的**业务逻辑层**；O7 设置中心承载**前端入口**，底层报告/通知子系统（§7.7.2–§7.7.5）承担**调度 + 推送 + 归档**。日报生成的代码路径被定时任务、设置中心手动按钮、AI 助手 Function Calling 三个入口共用。
...
> 整合「账号工作台 / 通知与邮件订阅 / AI 助手偏好 / 显示与界面」四个 tab。底层的报告生成 / 站内信 / 邮件发送 / APScheduler 子系统**全部保留**，只是前端入口集中到「通知与邮件订阅」tab + 顶部铃铛。原"报告中心独立页面"取消。

#### 7.7.1 页面结构（按原型 Board 9）

四个并列 tab，左侧顶部固定标题"设置中心"：

| Tab | 主要内容 |
|---|---|
| 账号工作台 | 当前账号、退出、API 用量统计、版本信息 |
| 通知与邮件订阅 | 邮件订阅开关、收件邮箱、订阅频率（日报/周报/关键事件）；底部最近 10 份历史报告列表 + 顶部「立即生成日报/周报」按钮；详细布局见 §7.7.6 |
| AI 助手偏好 | AI 模型档位（quick/strong）、Function Calling 开关、生成温度 |
| 显示与界面 | 主题切换、紧凑模式、图表偏好 |

底层报告/通知子系统（§7.7.2–§7.7.5、§7.7.7）原封不动保留——AI 助手仍会在每天/每周固定时间自动：聚合数据 → LLM 生成报告 → 入 `reports` 表 → 投递站内信（铃铛红点）→ HTML 邮件发送。
...
> 报告中心独立页面取消后，**铃铛成为运营端唯一的 AI 自主活动入口**：所有定时生成的报告、AI 助手触发的事件都通过铃铛红点 + 下拉列表进入对应详情页（如报告详情 `/ops/reports/:id`）。
...
│ ──────────────────────────────────────────────────────────       │
│                                                                  │
│ 最近 10 份报告                  [立即生成日报] [立即生成周报]    │
│ ┌────────────────────────────────────────────────────────────┐ │
│ │ 美甲品类日报 2026-05-26   日报  09:00   已发                │ │
│ │ 美甲品类周报 05-19~05-25  周报  09:00   已发                │ │
│ │ ...                                                          │ │
│ └────────────────────────────────────────────────────────────┘ │
└──────────────────────────────────────────────────────────────────┘
```

- 邮件订阅开关 / 收件邮箱 / 订阅频率仅前端存（`localStorage` 或后续单独的小 API），不复用 `.env` 里的 `REPORT_RECIPIENT`（后者是兜底默认收件人）
- 「立即生成日报/周报」按钮触发 `POST /api/ops/reports/generate?type=daily|weekly`，行为与原报告中心独立页面相同
- 历史报告列表点击行跳 `/ops/reports/:id`（详情页路由保留，但无独立列表页路由）
- 列表只显示最近 10 条；若需翻历史可通过 URL 直接访问详情页或后续增加简化全量列表（P2）
...
**为什么这样分**：
- 赛题只给了女款，男款是为支持 U0 性别维度差异化由用户从外部补充
...
| 路径 | 页面 |
|---|---|
| `/` | L0 双端入口（两张大卡片，分流到用户端或运营端） |
| `/gender` | U0 性别选择（用户端第一步） |
| `/upload` | U1 手图上传 |
| `/recommend` | U2 推荐 |
| `/browse` | U3 浏览 |
| `/compare` | U4 对比试戴 |
...
| `/ops/styles` | O6 |
| `/ops/setting` | O7 设置中心（4 tab：账号工作台 / 通知与邮件订阅 / AI 助手偏好 / 显示与界面） |
| `/ops/reports/:id` | 报告详情（仅从铃铛通知 / 设置中心邮件订阅 tab 内的历史报告列表跳转进入；无独立列表页路由） |
...
| D2 | L0 + 用户端 U0/U1/U2 跑通（含真实推荐算法 + LLM 理由） | 浏览器可完成"L0 选用户端 → 选性别 → 上传手图 → 看推荐"完整链路 |
| D3 | U4 多款对比试戴跑通（核心差异化）+ U3/U5 | 单设备可完成完整 C 端流程 |
| D4 | 运营端 O1/O2/O3/O4 全部跑通 + AI 助手 O5 + O7 设置中心（含「通知与邮件订阅」tab）+ 底层报告/通知子系统（APScheduler 定时 + 邮件 + 站内信） | 点"立即生成"后铃铛 5 秒内出现红点 + 邮箱真实收到 HTML 邮件 + reports 表新增一条 |
...
- **邮件发送若被网络封端口**：降级为站内信单通道，报告详情页显示"邮件发送失败"+ 重试按钮（不阻塞核心闭环）
...
### 13.3 AI 自主运营（报告与通知子系统）

运营不再需要主动查数据，AI 助手定时把分析结果推送过来：

- **早上 9 点** → 定时器触发，自动聚合昨日数据并 LLM 生成日报 → 写入 `reports` 表 + 站内信 + 发邮件
- **登录系统** → 顶部铃铛红点显示未读，下拉可见最近通知
- **每周一 9 点** → 同一链路触发周报（含周对比）
- **想看历史** → 设置中心 →「通知与邮件订阅」tab → 最近 10 份报告列表 → 点行进入详情页

整套机制不是"分析工具"，而是"会主动推送决策建议的虚拟运营助手"。报告/通知/调度子系统是这套自主性的底层支撑（详见 §7.7、§9.2 调度部分）。
...
│   ├── routers/
│   │   ├── user.py        # U0–U6 接口
│   │   └── ops.py         # O1–O7 接口（含报告/通知子系统接口、设置中心 API）
│   ├── services/
│   │   ├── recommend.py   # 推荐算法
