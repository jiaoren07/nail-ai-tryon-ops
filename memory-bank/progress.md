...
### ✅ Batch D · Step 10.1–10.4 端到端冒烟 + 收尾清单 — 2026-08-28

> 批式模式第四批（最终批）。**55 步全部完成，项目达到 plan 定义的"可交付最小完整状态"。**
> 真实 SMTP 发信经用户批准执行 1 次成功后，按用户指示重新隔离（默认拔线，需要时再接）。

**做了什么：**
- **10.1 数据闭环**：浏览器完整用户流（L0→男→上传样例→9 款推荐→3 次试戴→收藏）→ O1 页面轮询 KPI 538→541（+3 达标）、转化率同步动。批量脚本 60 次试戴 f_20 + 15 次收藏 → f_20 以 +1900% 增长冲上 O2 爆款榜首（三规则全过）。
- **10.2 生图切换**：环境变量覆盖（不碰 .env）切 seedream → 真实生成 808KB/25.4s（文件名 seedream_ 前缀）→ 切回 mock 2.2s 封面复制。两模式 result_url 均正常，切换零副作用，.env 始终未动。
- **10.3 报告全链路（真实邮件）**：正常配置重启 → 生成日报 #10（39.7s）→ **5 秒内 email_status=sent，真实邮件已发出**（收件箱 HTML 渲染待用户确认）→ 随即恢复 SMTP 隔离。
- **10.4 完整故事线**：O2 采纳 f_20 → 铃铛点开日报通知 → 详情页（已发送绿标 + 五章节完整）→ AI 助手 3 轮（TopN 表格 / f_15 vs f_20 对比引用精确数据 63 次 / boost f_09 动作卡 audit#48）。DB 终态账目吻合（今日 603 = 538+3+60+2）。
- **收尾清单 5/5**：依赖已锁定 ✓；.env.example 与 config 九字段对齐 ✓；**删库→seed→启动→全接口冒烟通过**（顺带清干净全部验证残留，reports/notifications 归零 = 干净演示态，旧库备份在会话 scratchpad）✓；根目录 README.md（四步启动）✓；无 _debug 残留 ✓。

**验证与 plan 口径的偏差（如实记录）：**
| 项 | plan 口径 | 实测 |
|---|---|---|
| / 路径 | "重定向 /upload" | L0 双端落地页（Step 5.2 已验收的后期设计，plan 未回写） |
| 10.1 性别 | 选女 | 实测走男流程（闭环等价；10.4 的男流程要求同时覆盖） |
| U2 选 3 款对比 | 加入对比→对比页 | 实测「试这款」为单款直达；3 次单款试戴等价完成闭环写入 |
| 报告生成时长 | <30s | 实测 39.7s（reasoning 模型，已在 HANDOFF 修正口径 <60s） |
| mock 文件应显著小于真图 | — | mock=封面复制 1193KB 反而大于真图 808KB；可靠区分是文件名 seedream_ 前缀 |
| Console 无 ERROR | 严格无 | 无应用级错误；存在 antd v6 弃用告警（已列清理项）+ 一条后端重启窗口期的网络失败 |

**性能实测（plan §10.4 口径）：** 推荐页数秒内出卡 ✓ / mock 试戴 2.2s ✓ / 看板即时 ✓ / 报告 39.7s（修正口径 <60s ✓）/ seedream 单张 25.4s（真实生图场景专用）。

**给后续开发者的提示：**
- **项目主线已完结。** 可选优化项（按需）：antd v6 弃用 API 统一替换（Tag bordered/Drawer width/Statistic valueStyle）；路由级 code splitting（主 chunk 2.4MB）；PPIO key 轮换（Step 4.5 遗留，用户操作）。
- 演示前标准动作：`seed_all.py`（锚定时间窗）+ 正常启动两端。要真实邮件就正常启动；不要就加 `$env:SMTP_HOST="smtp-disabled.invalid"`（用户已确认此默认姿态）。
- 当前库是干净交付态（零残留）；旧验证数据备份在会话 scratchpad `nail_demo_backup_20260828.db`，过期可删。
...
### ✅ Batch I · 试戴保真度修复（特写参考+V3 提示词）+ 找店预约移除 — 2026-09-13

背景：Render 切 seedream 后用户线上实测，f_21 的试戴结果几乎是款式封面的"重摄版"——同封面的握拳姿势+灰背景，用户上传的手被完全丢弃。用户同日拍板移除 U5「找店预约」占位按钮。

**做了什么：**
- **U5 找店预约按钮删除**：占位（点击仅弹"即将上线"toast），与 Batch H 删占位 tab 同一判断；按钮栏 5→4 列。
- **保真度两轮实验（promptlab）**：
  - 第一轮（仅改文字）：V1→V3（编辑任务式+"严禁照搬"负向约束），f_21 两版**都翻车**——强构图封面直接压过文字指令，纯提示词路线证伪。
  - 第二轮（换参考图）：把封面裁成指甲特写再喂，f_21 立刻保住用户的手；V3+特写还把方钻正确放上指甲（V1+特写把钻错放手背）→ 定案 = **特写参考 + V3 提示词**，用户验收通过。
- **工程化落地**：
  - `scripts/annotate_nail_bboxes.py`（新）：VLM（qwen3-vl-235b，与手图闸门同款）为 40 张封面标注指甲 bbox，0-1000 千分比→归一化，12% 外扩+面积合法性（4%~75%）校验，可断点续跑；结果写 `assets/dataset/nail_bboxes.json`（入仓的只有几 KB 的 JSON，不是裁剪图）。
  - 人工逐张复核裁剪 contact sheet：剔除 f_22（VLM 框到指甲油瓶）、f_24（框到整手+戒指）；m_07/m_09/m_15 三款"双手强构图"封面手动收紧到单手/局部指群（m_15 紧框实测由翻车转成功）。最终 **34 款用特写、6 款回退原封面**（f_16/f_22/f_24/m_02/m_03/m_11）。
  - `seed_styles.py`：seed 时按 JSON 用 PIL 现裁 `static/styles/{sid}_nail.jpg`（q90，毫秒级，冷启动零 VLM 依赖）。
  - `image_gen.py`：新增 `_resolve_style_ref_path()`（优先 `_nail.jpg`，缺则回退封面）；`PROMPT_TEMPLATE` 升级 V3；docstring 记录 V1→V3 演进依据。

**Batch I 验证：**
| 验证项 | 实测 |
|---|---|
| f_21（原翻车款）新管线 | 手+大理石背景完整保留，方钻/蓝边/闪粉迁移到指甲上 ✅ |
| f_15 复杂图案款 | 手保留，波点/花朵/格纹迁移 ✅ |
| m_15 男款（双手对指封面） | 全含裁剪仍翻车 → 紧框（单手列）后成功，五指设计各自迁移 ✅ |
| m_07 男款（十指相扣，最难构图） | 紧框后成功：猫咪/星星/爪印各就各位 ✅ |
| seed 重跑 | nail_crops=34 张现裁正常 |
| 前端 tsc/lint/build | 0 错（U4 两条既有 warning），构建 8.5s |

**几个设计选择（透明告知）：**
1. 入仓的是 bbox JSON 而非裁剪图：裁剪由 seed 现场完成（PIL 毫秒级），仓库不膨胀、Render 冷启动零外部依赖；改 bbox 只需改 JSON+reseed。
2. 特写不追求完美：目的只是"打碎封面的可照搬构图"，裁到少量手指皮肤无妨；裁不干净宁可回退原封面（6 款），不硬塞坏参考。
3. 设计还原不是像素级复刻：生成式路线的固有属性，"同一款美甲戴在你手上"成立即达标（用户验收口径）；分割+上色的像素级路线仍留在 roadmap 双模式。
4. 抽验策略：40 款没有逐款生成验证（¥8+40 分钟），选了风险分层——翻车复现款 f_21、复杂女款 f_15、双手男款 m_15/m_07 四个代表全过；如后续个别款仍翻车，改该款 bbox 或从 JSON 剔除即可，单点可修。

**给后续开发者的提示：**
- 新增款式的接入流程：封面入 assets → 跑 `annotate_nail_bboxes.py`（增量）→ 肉眼看一下裁剪 → reseed。
- 判断某款是否走了特写：`_resolve_style_ref_path(style_id)` 返回 `_nail.jpg` 即是；线上出图翻车优先查该款 bbox 而不是改提示词。
- V3 提示词单独救不了强构图封面（已证伪），别在文字上继续堆约束。

---

### ✅ Batch J · 双端切换补全（用户端 → 运营端） — 2026-09-13

用户反馈：Batch H 只做了运营端→用户端（侧栏按钮），反向缺失。新增共享组件 `components/SideSwitch.tsx`（「⇄ 运营端」胶囊，一键直达 /ops/overview），接入用户端 U0-U6 全部 7 页顶栏右侧（L0 本身是双端入口页不加；U6 的 Segmented 与其包进同一 flex 容器避免 justify-between 三分布局）。取直达按钮而非用户建议原话里的下拉/开关：两端仅两个选项，下拉多一步；与运营端侧栏按钮形成对称。验证：U0 点击直达 O1 看板、U2 组件在位、tsc/lint/build 全绿、console 无新错。设计立场：真实消费产品不会暴露运营入口，此处是双端演示产品的刻意取舍（组件注释已写明）。
