...
两条路径，代码侧改造（单一来源托管、自愈 seed、生图额度护栏）都已完成：

| | A. Render 免费层（本文第一部分） | B. 国内云主机（第二部分） |
|---|---|---|
| 成本 | 免费 | 新人活动 ¥99/年档 |
| 体验 | 闲置 15 分钟休眠，冷启动 30–60 秒；新加坡节点 | 秒开，无休眠 |
| 适合 | 简历上"有个能点的链接" | 要求点开即用的正式展示 |

---

# A. Render 免费层（Docker，零成本）

仓库里已备好 `Dockerfile` / `render.yaml` / `backend/scripts/start.sh`，
**不需要改任何代码**，全程在网页点击完成。

## A1. 部署步骤

1. 打开 https://render.com → **Get Started** → 选 **Sign in with GitHub** 授权登录
2. 顶部 **New +** → **Blueprint**
3. 选中仓库 `jiaoren07/nail-ai-tryon-ops` → **Connect**
   （Render 会自动读取根目录的 `render.yaml`，服务名/区域/环境变量都已预填）
4. 页面会要求填一个变量：**PPIO_API_KEY** → 粘贴你的 PPIO key（存在 Render 加密变量里，不进仓库）
5. **Apply** → 首次构建约 5–10 分钟（装依赖 + 构建前端）
6. 完成后页面顶部出现网址，形如 `https://nail-ai-tryon-ops.onrender.com` —— 这就是可以写进简历的链接

## A2. 免费层的已知行为（不是故障）

- **休眠**：连续 15 分钟无人访问就休眠；下一个访客点开要等 30–60 秒冷启动（页面转圈后正常打开）
- **数据每次冷启动重置**：免费层磁盘是临时的，容器每次启动自动重新 seed。
  这反而是优点——演示数据永远新鲜、时间窗口自动对齐到"现在"，
  访客在运营端做的任何改动（下架/调序）都会在下次冷启动后复原
- **试戴默认是 mock**（结果图为款式封面，界面已如实标注「快速预览模式」）。
  想让访客看到真实 AI 合成：Render 面板 → Environment → 把 `IMAGE_PROVIDER` 改成 `seedream` →
  Save 后自动重部署。费用按 PPIO 实际调用计，`SEEDREAM_DAILY_QUOTA=20` 是每日封顶

## A3. 后续维护

- 往 GitHub main 分支推代码 → Render 自动重新部署，无需手动操作
- 看日志：Render 面板 → 你的服务 → **Logs**
- 改配置：面板 → **Environment** → 改完 Save 即自动重部署
